...
for _ in range(0, 3):
    ...
    NAME = "sin-gan"  # Modification of SDI-GAN, selective intensity gan
    ...
    x_train, x_test, x_train_2, x_test_2, y_train, y_test, std_train, std_test, intensity_train, intensity_test = train_test_split(data, data_2, data_cond, std, proton_photon_sum, test_size=0.2, shuffle=False)
    ...
    dataset_with_cond = tf.data.Dataset.zip((dataset, dataset_2, dataset_cond, dataset_std, dataset_intensity, fake_cond)).shuffle(12800)
    ...
    val_dataset_with_cond = tf.data.Dataset.zip((val_dataset, val_dataset_2, val_dataset_cond, val_dataset_std, val_dataset_intensity, val_fake_cond)).shuffle(12800)
    ...
    def train_step(batch, step):
        images, images_2, cond, std, intensity, noise_cond = batch
        step = step
        ...
        return gen_loss, disc_loss, div_loss, intensity_loss
    ...
    def train(dataset, epochs):
        ...
        for epoch in range(epochs):
            ...
            for batch in dataset:
                gen_loss, disc_loss, div_loss, intensity_loss = train_step(batch, tf_step)
